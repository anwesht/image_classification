...
#WORK_DIRECTORY = 'data'
WORK_DIRECTORY = '/share/apps/MNIST_data'
#local_dir = './'
IMAGE_SIZE = 28
NUM_CHANNELS = 1
PIXEL_DEPTH = 255
...
def main(argv=None):  # pylint: disable=unused-argument
  if FLAGS.self_test:
    ...
    # Extract it into numpy arrays. *** Changed to 30000 training examples
    train_data = extract_data(train_data_filename, 30000)
    train_labels = extract_labels(train_labels_filename, 30000)
...
#      shape=(EVAL_BATCH_SIZE, IMAGE_SIZE, IMAGE_SIZE, NUM_CHANNELS))
  ...
  conv1_weights = tf.Variable(
      tf.truncated_normal([5, 5, NUM_CHANNELS, 32],  # 3x3 filter, depth 32.
                          stddev=0.1,
                          seed=SEED, dtype=data_type()))
  #conv1_biases = tf.Variable(tf.zeros([32], dtype=data_type()))
  conv1_biases = tf.Variable(tf.constant(0.1, shape=[32], dtype=data_type()))
  conv2_weights = tf.Variable(tf.truncated_normal(
      [5, 5, 32, 64], stddev=0.1,
      seed=SEED, dtype=data_type()))  # also 3x3 says we connect 32 to 64 (features)
...
#output will have shape [None, 28, 26, 16], because you have only a padding of 1 in width.
    # *** Super critical is that padding is used so that the convolution output is the same size as
    # the original
...
#  optimizer = tf.train.MomentumOptimizer(learning_rate,
 #                                        0.9).minimize(loss,
  ...
  optimizer = tf.train.AdamOptimizer(learning_rate,
                                         BETA1_MOMENT, BETA2_MOMENT).minimize(loss,
                                                       global_step=batch)
  ...
  with tf.Session() as sess:
    ...
    tf.initialize_all_variables().run()
    print('Initialized!')
    print("num_epochs = {}, train_size = {}, BATCH_SIZE = {}".format(num_epochs, train_size, BATCH_SIZE))
    print("LEARNING_RATE = {}".format(LEARNING_RATE))
    print("DECAY_RATE = {}".format(DECAY_RATE))
    print("BETA1_MOMENT = {}".format(BETA1_MOMENT))
    print("BETA2_MOMENT = {}".format(BETA2_MOMENT))
